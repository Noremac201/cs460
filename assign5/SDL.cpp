...
{  // intersect the ray with the TaperedCylinder
		...
		for (int i=0;i<num;i++)
		{
			P0 = rayPos(genRay,inter.hit[i].hitTime);
...
{  // intersect the ray with the generic sphere
		...
		if(tIn>0.00001)// is first hit in front of the eye
		{
			...
			inter.hit[0].hitObject=this;
			inter.hit[0].hitPoint = rayPos(genRay.start,genRay.dir,tIn);
			inter.hit[0].hitNormal = cubeNormal(inSurf);
			num++;// have a hit
		}
		if(tOut>0.00001)
		{
			...
			inter.hit[num].hitPoint = rayPos(genRay.start,genRay.dir,tOut);
			inter.hit[num].hitNormal = cubeNormal(outSurf);
			num++;
		}
		inter.numHits=num;
		return (num>0);
...
bool Mesh::hit(Ray &r, Intersection &inter)
{
	...
	if(tIn > 0.00001) //is first hit in front of the eye?
	{
		...
		inter.hit[num].hitObject = this;
		inter.hit[num].hitPoint = rayPos(genRay.start, genRay.dir,tIn);
		inter.hit[num].hitNormal = norm[face[inSurf].vert[0].normIndex];
		num++; //got a hit
	}
	if(tOut > 0.00001)
	{
		...
		inter.hit[num].hitObject = this;
		inter.hit[num].hitPoint = rayPos(genRay.start,genRay.dir,tOut);
		inter.hit[num].hitNormal = norm[face[outSurf].vert[0].normIndex];
		num++;
	}
...
Color3 Scene :: texture(Shape* Obj,Point3 &p,HitInfo hit)
{
	...
	//start the pixmap mapping
		...
		int N = pixmap[i].nCols;
        int M = pixmap[i].nRows;
        mRGB rgb;
		rgb = pixmap[i].getPixel((int)(u*N),(int)(v*M));
